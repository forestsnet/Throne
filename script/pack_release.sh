...
build_installer() {
    ...
    # Имя без версии: ссылка на releases/latest/download/<имя> не ломается.
    cp "$DEPLOY/Throne-$TAG-windows-universal-installer.exe" "$DEPLOY/FSNT-Client-Setup.exe"
}
...
# Образы приехали из build-cpp: hdiutil есть только на маке.
name_dmg() {
    cd "$DEPLOY"
    mv darwin-arm64/FSNT-Client.dmg FSNT-Client-macos-arm64.dmg
    mv darwin-amd64/FSNT-Client.dmg FSNT-Client-macos-intel.dmg
    mv darwinlegacy-amd64/FSNT-Client.dmg FSNT-Client-macos-intel-legacy.dmg
}
...
export -f build_installer zip_dir zip_app name_dmg zip_debug pack_deb pack_rpm run_task
