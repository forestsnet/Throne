...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    // debug info shortcut
    connect(shortcut_debug_info, &QShortcut::activated, this, &MainWindow::on_menu_collect_debug_info_triggered);
...
void MainWindow::set_spmode_vpn(bool enable, bool save) {
    ...
    // Но только если VPN включается (enable = true). Если выключается - просто останавливаем.
    if (enable && Configs::dataManager->settingsRepo->started_id >= 0) {
...
void MainWindow::on_menu_collect_debug_info_triggered() {
    ...
    runOnNewThread([=, this] {
        ...
        // 7. Логи приложения
        QString appLogsPath = tempDir + "/07_app_logs.txt";
        QString appLogFilePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/throne_debug.log";
        QFile appLogFile(appLogFilePath);
        if (appLogFile.exists()) {
            QFile::copy(appLogFilePath, appLogsPath);
        }
        ...
        if (settingsFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            stream << "Core port: " << settings->core_port << "\n";
            ...
            stream << "Remote DNS strategy: " << settings->remote_dns_strategy << "\n";
            stream << "Direct DNS: " << settings->direct_dns << "\n";
            stream << "Direct DNS strategy: " << settings->direct_dns_strategy << "\n";
            stream << "Use DNS object: " << (settings->use_dns_object ? "Yes" : "No") << "\n";
            stream << "DNS final out: " << settings->dns_final_out << "\n";
            stream << "Domain strategy: " << settings->domain_strategy << "\n";
            stream << "Outbound domain strategy: " << settings->outbound_domain_strategy << "\n";
            stream << "Sniffing mode: " << settings->sniffing_mode << "\n";
